...
export const useDensityVisualization = (particles, particleCount, binSize, dimension = '2D', useGPU = false, particlesLoaded) => {
    const canvasRef = useRef(null);
    const [densityData2D, setDensityData2D] = useState(null);
    const [densityData1D, setDensityData1D] = useState(null);
    // Remove particlesRef pattern - use particles directly for proper reactivity
    const drawDensityHeatmap = useCallback((data) => {
    ...
    const drawDensity1D = useCallback((data) => {
        const canvas = canvasRef.current;
        if (!canvas)
            return;
        const ctx = canvas.getContext('2d');
    ...
    const updateDensity = useCallback(() => {
        ...
        if (useGPU && particlesLoaded) {
            try {
                const gpuManager = particlesLoaded.getGPUManager?.();
                if (gpuManager) {
