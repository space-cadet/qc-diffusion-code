...
export function useRandomWalkPanels() {
  ...
  const handleObservablesDragStop = useCallback((x: number, y: number) => {
    setObservablesWindow({ ...observablesWindow, left: x, top: y });
  }, [observablesWindow, setObservablesWindow]);

  const handleObservablesResizeStop = useCallback((width: number, height: number, x: number, y: number) => {
  ...
  const handleCustomObservablesDragStop = useCallback((x: number, y: number) => {
    setCustomObservablesWindow({ ...customObservablesWindow, left: x, top: y });
  }, [customObservablesWindow, setCustomObservablesWindow]);

  const handleCustomObservablesResizeStop = useCallback((width: number, height: number, x: number, y: number) => {
