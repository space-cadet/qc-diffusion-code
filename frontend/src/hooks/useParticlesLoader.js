...
export const useParticlesLoader = ({ simulatorRef, tsParticlesContainerRef, gridLayoutParamsRef, gridLayoutParams, renderEnabledRef, timeRef, collisionsRef, useGPU }) => {
    ...
    const startAnimation = useCallback((container) => {
        ...
        const animate = () => {
            ...
            if (shouldRender && container) {
                if (useGPU && gpuManagerRef.current) {
                    ...
                    if (simulatorRef.current && gpuManagerRef.current.setCanvasMapper && !gpuManagerRef.current._mapperSet) {
                        const pm = simulatorRef.current.getParticleManager?.();
                        if (pm) {
                            gpuManagerRef.current.setCanvasMapper((pos) => pm.mapToCanvas(pos));
                            gpuManagerRef.current._mapperSet = true;
    ...
    const particlesLoader = useCallback((container) => {
        ...
        if (useGPU && !gpuManagerRef.current) {
            ...
            const htmlCanvas = container?.canvas?.element;
            ...
            if (htmlCanvas && container.particles && container.particles.count > 0) {
                try {
                    ...
                    window.gpuManager = gpuManagerRef.current;
                }
                catch (error) {
        ...
        container._restartAnimation = restartAnimation;
    }, [cleanup, startAnimation, restartAnimation]);
    // Expose GPU methods
    particlesLoader.resetGPU = resetGPU;
    particlesLoader.initializeGPU = initializeGPU;
    particlesLoader.updateGPUParameters = updateGPUParameters;
    particlesLoader.getGPUManager = () => gpuManagerRef.current;
