...
export const setParticleManager = (manager) => {
...
export const createParticleContainer = async (canvasElement, particleCount) => {
    if (!engine) {
...
export const updateParticlesFromStrategies = (container, showAnimation = true, isSimulationRunning = false) => {
    if (!particleManager || !showAnimation) {
        return;
    }
    const physicsParticles = particleManager.getAllParticles();
    ...
    if (tsCount > 0) {
        for (let i = 0; i < tsCount; i++) {
            const tsParticle = particlesContainer.get(i);
            if (!tsParticle)
                continue;
            ...
            if (physicsParticle) {
                ...
                if (tsParticle.opacity?.value !== undefined) {
                    tsParticle.opacity.value = 0.8;
                }
    ...
    if (_diagFrameCounter % 60 === 0 && isSimulationRunning) {
        ...
        const firstTsParticle = container.particles.get(0);
...
// Clean shutdown
export const destroyContainer = (container) => {
    container.destroy();
};
