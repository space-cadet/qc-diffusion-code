...
export default function Controls({ params, onChange }) {
    ...
    const handleSolverChange = (equation, solver) => {
    ...
    return (<div className="p-4 bg-gray-50 border-r border-gray-200">
      ...
      <div className="mb-6">
        ...
        {solverOpen && (<div className="mt-2 p-3 bg-white rounded-lg border border-gray-200 space-y-3">
            <div>
              <label className="block text-sm font-medium mb-1">Type</label>
              <select value={params.solver_type ?? 'python'} onChange={(e) => onChange({ ...params, solver_type: e.target.value })} className="w-full p-2 border border-gray-300 rounded text-sm">
                <option value="python">CPU (Backend Python)</option>
                <option value="webgl">GPU (WebGL Explicit)</option>
              </select>
            </div>
            {params.solver_type === 'webgl' && (<div className="text-xs text-gray-600">
            ...
            <div className="p-3 bg-white rounded-lg border border-gray-200">
              ...
              {pdeUIState.telegraphOpen && (<div className="mt-3 ml-2 space-y-3 border-l-2 border-gray-200 pl-3">
                  <div>
                    ...
                    <input type="range" min="0.001" max="3" step="any" value={params.collision_rate} onChange={(e) => handleChange('collision_rate', parseFloat(e.target.value))} className="w-full"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1">
                      Velocity (v): {params.velocity.toFixed(2)}
                    </label>
                    <input type="range" min="0.5" max="3" step="any" value={params.velocity} onChange={(e) => handleChange('velocity', parseFloat(e.target.value))} className="w-full"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1">Solver</label>
                    <select value={params.solver_config?.telegraph ?? 'forward-euler'} onChange={(e) => handleSolverChange('telegraph', e.target.value)} className="w-full p-2 border border-gray-300 rounded text-sm">
            ...
            <div className="p-3 bg-white rounded-lg border border-gray-200">
              ...
              {pdeUIState.diffusionOpen && (<div className="mt-3 ml-2 space-y-3 border-l-2 border-gray-200 pl-3">
                  <div>
                    ...
                    <input type="range" min="0.001" max="2" step="any" value={params.diffusivity} onChange={(e) => handleChange('diffusivity', parseFloat(e.target.value))} className="w-full"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1">Solver</label>
                    <select value={params.solver_config?.diffusion ?? 'crank-nicolson'} onChange={(e) => handleSolverChange('diffusion', e.target.value)} className="w-full p-2 border border-gray-300 rounded text-sm">
                      <option value="crank-nicolson">Crank-Nicolson</option>
                      <option value="forward-euler">Forward Euler</option>
                      <option value="rk4">Runge-Kutta 4</option>
                    </select>
      ...
      {(params.solver_config?.telegraph === 'crank-nicolson' ||
            ...
            params.solver_config?.diffusion === 'rk4') && (<div className="mb-6 p-3 bg-white rounded-lg border border-gray-200">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min="0.1" max="2.0" step="0.1" value={params.solver_params?.dt_factor ?? 1.0} onChange={(e) => onChange({
                ...params,
            ...
            {(params.solver_config?.telegraph === 'crank-nicolson' ||
                ...
                <div>
                  ...
                  <input type="number" min="0" max="1" step="0.1" value={params.solver_params?.theta ?? 0.5} onChange={(e) => onChange({
                    ...params,
                ...
                <div>
                  ...
                  <input type="number" min="1e-8" max="1e-3" step="1e-7" value={params.solver_params?.tolerance ?? 1e-6} onChange={(e) => onChange({
                    ...params,
                ...
                <div>
                  ...
                  <input type="number" min="10" max="1000" step="10" value={params.solver_params?.max_iter ?? 100} onChange={(e) => onChange({
                    ...params,
      ...
      <div className="mb-6 p-3 bg-white rounded-lg border border-gray-200">
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Type</label>
          <select value={params.boundaryCondition ?? 'neumann'} onChange={(e) => onChange({ ...params, boundaryCondition: e.target.value })} className="w-full p-2 border border-gray-300 rounded text-sm">
            <option value="neumann">Neumann (zero gradient)</option>
            <option value="dirichlet">Dirichlet (fixed value)</option>
          </select>
        </div>
        {params.boundaryCondition === 'dirichlet' && (<div className="mt-3">
            <label className="block text-sm font-medium mb-1">Boundary Value</label>
            <input type="number" step="0.1" value={params.dirichlet_value ?? 0.0} onChange={(e) => onChange({ ...params, dirichlet_value: parseFloat(e.target.value) })} className="w-full p-2 border border-gray-300 rounded text-sm"/>
          </div>)}
      </div>
      ...
      <div className="mb-6 p-3 bg-white rounded-lg border border-gray-200">
        ...
        {pdeUIState.simulationSettingsOpen && (<div className="space-y-4">
          ...
          <div>
            ...
            <input type="range" min="-10" max="0" step="any" value={params.x_min} onChange={(e) => handleChange('x_min', parseFloat(e.target.value))} className="w-full"/>
          </div>
          ...
          <div>
            ...
            <input type="range" min="0" max="10" step="any" value={params.x_max} onChange={(e) => handleChange('x_max', parseFloat(e.target.value))} className="w-full"/>
          </div>
          ...
          <div>
            ...
            <input type="range" min="0.001" max="100" step="any" value={params.t_range} onChange={(e) => handleChange('t_range', parseFloat(e.target.value))} className="w-full"/>
          </div>
