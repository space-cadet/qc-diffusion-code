...
const QuantumWalkPage = () => {
    ...
    const [coinType, setCoinType] = useState('hadamard');
    ...
    const [classicalModel, setClassicalModel] = useState('simple');
    ...
    const quantumStateRef = useRef(null);
    const classicalStateRef = useRef(null);
    const timerRef = useRef(null);
    // --- Utility Functions ---
    const calculateSpreadWidth = (probabilities) => {
    ...
    const determineRegime = (step, size, variance) => {
        if (step === 0)
            return 'Initial';
        if (Math.sqrt(variance) > size / 4)
    ...
    const calculateVariance = (values, weights) => {
        if (values.length === 0)
            return 0;
    ...
    const extractQuantumData = (state, size, step) => {
        const probabilities = [];
        const rawProbs = [];
        const x0 = (size - 1) / 2;
        let maxProb = 0;
    ...
    const extractClassicalData = (probs, size, step) => {
        const x0 = (size - 1) / 2;
    ...
    // --- Operator Builders ---
    const buildCoinOperator = useCallback((type, angle) => {
        if (type === 'hadamard') {
    ...
    const buildShiftOperator = useCallback((size, boundary) => {
        const dim = 2 * size;
        ...
        for (let pos = 0; pos < size; pos++) {
            ...
            if (boundary === 'periodic') {
                ...
                if (pos > 0)
                    ...
                    setSparseEntry(matrix, 0 + size, leftIdx, math.complex(1, 0)); // Reflect
            }
            ...
            if (boundary === 'periodic') {
                ...
                if (pos < size - 1)
                    ...
                    setSparseEntry(matrix, size - 1, rightIdx, math.complex(1, 0)); // Reflect
            }
        }
    ...
    const stepWalk = useCallback(() => {
        ...
        const identityMatrix = Array.from({ length: size }, (_, i) => Array.from({ length: size }, (_, j) => i === j ? math.complex(1, 0) : math.complex(0, 0)));
    ...
    const seekToStep = (step) => {
        ...
        if (qHistory)
            setQuantumData(qHistory.data);
        if (classicalStateRef.current) {
            const cHistory = classicalStateRef.current.history.find(h => h.step === step);
            if (cHistory)
                setClassicalData(cHistory.data);
        }
        setCurrentStep(step);
    };
    useEffect(() => {
        if (isRunning)
            timerRef.current = setInterval(stepWalk, ANIMATION_DELAY);
    ...
    if (activeTab !== 'quantumwalk')
        return null;
    return (<div className="flex flex-col h-screen bg-gray-100 font-sans overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={`${isSidebarCollapsed ? 'w-16' : 'w-72'} bg-white border-r border-gray-200 transition-all duration-300 flex flex-col flex-shrink-0`}>
          ...
          {!isSidebarCollapsed && (<div className="p-4 border-t border-gray-200 space-y-4">
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <select value={coinType} onChange={e => setCoinType(e.target.value)} className="text-xs p-1.5 border border-gray-200 rounded bg-white w-full">
        ...
        <main className="flex-1 p-6 bg-gray-50 overflow-y-auto">
          {currentView === 'visualization' && (<div className="space-y-6 max-w-6xl mx-auto">
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select value={boundaryType} onChange={e => setBoundaryType(e.target.value)} className="p-2 border border-gray-300 rounded text-sm outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select value={classicalModel} onChange={e => setClassicalModel(e.target.value)} className="p-2 border border-gray-300 rounded text-sm outline-none focus:ring-2 focus:ring-blue-500">
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select value={coinType} onChange={e => setCoinType(e.target.value)} className="p-2 border border-gray-300 rounded text-sm outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div className="lg:col-span-3 space-y-4">
                  <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        ...
                        {compareClassical && (<label className="flex items-center gap-2 text-xs font-medium text-gray-600 cursor-pointer select-none">
                  ...
                  <div className="bg-white rounded-lg border border-gray-200 shadow-md overflow-hidden">
                    ...
                    legend: { orientation: 'h', x: 0.5, xanchor: 'center', y: 1.1 }
                      ...
                      {compareClassical && !overlayClassical && (<div className="border-t border-gray-100 pt-6">
                          <h4 className="text-sm font-bold text-gray-700 mb-4 uppercase">Classical Walk Comparison</h4>
                          <div className="aspect-[16/9]">
                            {classicalData && (<Plot data={[{
                            x: classicalData.probabilities.map(p => p.position),
                            y: classicalData.probabilities.map(p => p.probability),
                            type: 'bar' as any,
                            name: 'Classical Probabilities',
            ...
            ].map((stat, i) => (<div key={i} className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{stat.label}</p>
                      <p className={`text-xl font-bold mt-2 ${stat.color === 'blue' ? 'text-blue-600' :
          ...
          {currentView === 'analysis' && (<div className="space-y-6 max-w-6xl mx-auto pb-12">
                ...
                <div className="p-0 overflow-x-auto">
                  <table className="w-full text-left bg-white border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {varianceAnalysis.filter((_, i) => i % 5 === 0 || i === varianceAnalysis.length - 1).map((row, i) => (<tr key={i} className="hover:bg-blue-50/30 transition-colors">
          ...
          {currentView === 'education' && (<div className="space-y-8 max-w-4xl mx-auto pb-12">
              <div className="bg-white rounded-lg border border-gray-200 shadow-md overflow-hidden">
