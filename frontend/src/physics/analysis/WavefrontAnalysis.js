function calculateCenterOfMass(density) {
    let totalMass = 0;
    let xSum = 0;
    let ySum = 0;
...
export function analyzeWaveFrontSpeed(densityHistory, currentStrategy) {
    ...
    for (let i = 1; i < recent.length; i++) {
        ...
        if (dt > 0) {
            ...
            const distance = Math.sqrt(Math.pow(currCM.x - prevCM.x, 2) + Math.pow(currCM.y - prevCM.y, 2));
