export function getDensityProfile2D(particles, particleCount, binSize = 10) {
    ...
    const density = Array(yBins).fill(null).map(() => Array(xBins).fill(0));
    // Bin particles
    ...
    const normalizedDensity = density.map(row => row.map(count => count / (particleCount * binArea)));
...
export function getDensityProfile1D(particles, particleCount, binSize = 10) {
    const positions = particles.map(p => p.position);
    if (positions.length === 0) {
        return {
            density: [],
    ...
    const density = Array(xBins).fill(0);
    // Bin particles
    positions.forEach(pos => {
...
export function getDensityField1D(particles, particleCount, dx = 0.1, maxBins = 1000) {
    const positions = particles.map(p => p.position);
