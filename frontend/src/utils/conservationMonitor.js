export class ConservationMonitor {
    ...
    computeQuantities(frame, params) {
        const quantities = {
            time: frame.time,
            mass_telegraph: 0,
            mass_diffusion: 0,
        ...
        selectedEquations.forEach(equationType => {
            const data = frame[equationType];
            if (!data || !data.x || !data.u)
                return;
    ...
    computePotentialEnergy(values, dx, velocity) {
        if (values.length < 2)
            return 0;
        let energy = 0;
    ...
    computeGradientEnergy(values, dx) {
    ...
    isStable(tolerance = 0.1) {
        const errors = this.getConservationErrors();
        return (errors.mass_telegraph_error < tolerance &&
