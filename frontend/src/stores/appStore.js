...
export const useAppStore = create()(persist((set) => ({
    ...
    updateSimulationMetrics: (time, collisions, status, interparticleCollisions) => set((state) => ({
        randomWalkSimulationState: {
    ...
    saveSimulationSnapshot: (particleData, densityHistory, observableData, gpuState) => set((state) => ({
        randomWalkSimulationState: {
            ...state.randomWalkSimulationState,
            particleData,
            densityHistory,
    ...
    migrate: (state, version) => {
        ...
        if (glp.minParticles === undefined)
            glp.minParticles = 0;
        if (glp.maxParticles === undefined)
            glp.maxParticles = 2000;
        if (glp.dt === undefined)
            glp.dt = 0.01;
        const ui = state.randomWalkUIState || {};
        if (ui.particlesLogScale === undefined)
            ui.particlesLogScale = true;
        return {
            ...state,
            gridLayoutParams: {
                ...glp,
