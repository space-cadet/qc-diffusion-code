...
function DraggableItem({ id, children, className }) {
    ...
    return (<div ref={setNodeRef} style={style} {...listeners} {...attributes} className={`cursor-grab active:cursor-grabbing ${className}`}>
      {children}
    </div>);
}
function DroppableZone({ id, children, items }) {
    const { isOver, setNodeRef } = useDroppable({ id });
    return (<div ref={setNodeRef} className={`min-h-32 p-4 border-2 border-dashed rounded-lg transition-colors ${isOver ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}`}>
      <div className="text-sm font-medium text-gray-700 mb-2">{children}</div>
      <div className="space-y-2">
        {items.map(itemId => (<div key={itemId} className="text-xs text-gray-500">Item: {itemId}</div>))}
      </div>
    </div>);
}
export default function RandomWalkPage() {
    ...
    const handleDragEnd = (event) => {
        ...
        const activeContainer = Object.keys(containers).find(key => containers[key].includes(activeId));
    ...
    return (<DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="h-screen flex flex-col">
        ...
        {Object.entries(containers).map(([containerId, items]) => (<div key={containerId} className="absolute">
            {items.map((itemId) => (<DraggableItem key={itemId} id={itemId} className={`p-3 m-1 border rounded-lg shadow-sm ${getItemColor(itemId)}`}>
        ...
        <DragOverlay>
          {activeId ? (<div className={`p-3 border rounded-lg shadow-lg ${getItemColor(activeId)}`}>
