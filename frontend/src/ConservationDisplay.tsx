...
export default function ConservationDisplay({ currentQuantities, errors, isStable, dtInfo, equationParams }) {
    if (!currentQuantities) {
        return (<div className="mt-3 p-3 bg-gray-50 rounded border text-sm">
    ...
    return (<div className="mt-3 p-3 bg-gray-50 rounded border text-sm">
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-base font-semibold text-gray-800">Conserved Quantities</h4>
        <span className={`px-2 py-1 rounded text-sm font-medium ${isStable ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
      ...
      {/* dt diagnostics */}
      {dtInfo && (<div className="mt-3 p-3 rounded bg-white border text-gray-800">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {typeof dtInfo.dt_limit_diff === 'number' && (<div>Diffusion limit: <span className="font-mono text-[0.95rem]">{dtInfo.dt_limit_diff.toExponential(3)}</span></div>)}
            {typeof dtInfo.dt_limit_tel === 'number' && (<div>Telegraph limit: <span className="font-mono text-[0.95rem]">{dtInfo.dt_limit_tel.toExponential(3)}</span></div>)}
          </div>
        </div>)}

      {equationParams && (<div className="mt-3 p-3 rounded bg-white border text-gray-800">
          ...
          <div className="space-y-2">
            ...
            {equationParams.telegraph && (<div className="grid grid-cols-2 gap-3">
            ...
            {equationParams.diffusion && (<div>Diffusion k (diffusivity): <span className="font-mono">{equationParams.diffusion.diffusivity.toExponential(3)}</span></div>)}
          </div>
        </div>)}
    </div>);
}
