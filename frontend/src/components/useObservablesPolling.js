...
export function useObservablesPolling(simulatorRef, visibleObservables, customObservableConfigs, isRunning, simReady) {
    const [observableData, setObservableData] = useState({});
    const pollingStatesRef = useRef({});
    const intervalRef = useRef(null);
    // Round interval to nearest multiple of DELTA_MIN
    const roundToMultiple = useCallback((interval) => {
    ...
    const pollObservable = useCallback((observableId) => {
