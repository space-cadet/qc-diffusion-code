...
export const LogNumberSlider = ({ value, onChange, min, max, label, logScale, onToggleLogScale, defaultLogScale = false, showNumberInput = true, step = 1, precision = 0, disabled = false, format, parse, className, id, dataTestId, discrete = false, }) => {
    ...
    return (<div className={className} id={id} data-testid={dataTestId}>
      ...
      <div className="flex items-center justify-between gap-2 mb-2">
        ...
        {showNumberInput && (<input type="number" min={min} max={max} step={discrete ? 1 : (step === 'any' ? 'any' : step)} value={value} onChange={(e) => onNumberChange(e.target.value)} className="w-28 border rounded px-2 py-1 text-sm" disabled={isDisabled}/>)}
      </div>
