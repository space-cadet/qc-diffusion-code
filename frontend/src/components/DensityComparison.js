...
export const DensityComparison = ({ particles, particleCount, simulatorRef, gridLayoutParams, simulationState, particlesLoaded, }) => {
    ...
    const totalBins = densityData ? (gridLayoutParams.dimension === '1D' ? densityData.density.length : densityData.density.length * densityData.density[0]?.length || 0) : 0;
    ...
    return (<div className="bg-white border rounded-lg p-4 h-full flex flex-col">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setAutoUpdate(!randomWalkUIState.densityAutoUpdate)} className={`px-3 py-1 text-xs rounded ${randomWalkUIState.densityAutoUpdate
      ...
      <div className="flex-1 flex gap-4">
        ...
        <div className="w-48 text-xs space-y-2">
          ...
          <div className="bg-gray-100 p-2 rounded">
            ...
            {densityData && gridLayoutParams.dimension === '2D' && densityData.density && (<div>Size: {densityData.density[0]?.length || 0}×{densityData.density.length}</div>)}
            {densityData && gridLayoutParams.dimension === '1D' && densityData.density && (<div>Size: {densityData.density.length}</div>)}
          </div>
        </div>
      </div>
    </div>);
