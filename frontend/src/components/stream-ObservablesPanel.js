...
function ObservableDisplay({ config, data, isVisible, onToggle }) {
    const formatValue = (value, format, precision) => {
...
export function StreamObservablesPanel({ simulatorRef, simReady }) {
    ...
    const visibleCustomIds = useMemo(() => customObservableConfigs
        .filter(c => c.valid && customObservableVisibility[c.name])
        .map(c => `text_${c.name}`), [customObservableConfigs, customObservableVisibility]);
    ...
    useEffect(() => {
        ...
        if (!(manager instanceof StreamObservableManager))
            return;
            ...
            if (!prevVisible.has(id) && !manager.hasObserver(id)) {
                try {
                    if (BUILT_IN_OBSERVABLES[id]) {
                        ...
                        const observable = new TextObservable(definition, simulatorRef.current.getParticleManager().getCoordinateSystem().getBoundaries());
                        observable.id = id;
    ...
    return (<div className="p-4 space-y-4">
      ...
      {Object.entries(BUILT_IN_OBSERVABLES).map(([id, config]) => (<ObservableDisplay key={id} config={config} data={observableData[id]} isVisible={visibleBuiltInIds.includes(id)} onToggle={(visible) => setRandomWalkUIState({ [config.uiToggle]: visible })}/>))}
      {customObservableConfigs.map((config, index) => {
            const observableId = `text_${config.name}`;
            return (<ObservableDisplay key={`custom-${index}`} config={{
                    id: observableId,
                    name: config.name,
                ...
                }} data={observableData[observableId]} isVisible={visibleCustomIds.includes(observableId)} onToggle={(visible) => setCustomObservableVisibility(config.name, visible)}/>);
        })}
    </div>);
}
