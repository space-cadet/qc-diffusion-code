 ...
 * @throws Error if matrix is invalid or non-square
 */
export function isHermitian(matrix, tolerance = NUMERICAL_THRESHOLD) {
    const validation = validateMatrix(matrix);
    if (!validation.valid) {
        throw new Error(`Invalid matrix: ${validation.error}`);
    }
 ...
 * @throws Error if matrix is invalid or non-square
 */
export function isUnitary(matrix, tolerance = NUMERICAL_THRESHOLD) {
    const validation = validateMatrix(matrix);
    if (!validation.valid) {
        throw new Error(`Invalid matrix: ${validation.error}`);
    }
 ...
 */
export function orthogonalizeStateVectors(stateVectors, precision = NUMERICAL_THRESHOLD) {
    ...
    for (let i = 0; i < orthogonalizedMatrix.length && i < stateVectors.length; i++) {
        ...
        const newState = new StateVector(dimension, orthogonalizedAmplitudes, originalState.basis ? `orth(${originalState.basis})` : 'orthogonalized', originalState.properties);
