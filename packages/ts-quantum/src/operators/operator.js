 ...
 */
export function createZeroMatrix(dimension) {
...
export class MatrixOperator {
    constructor(matrix, type = 'general', validateTypeConstraints = true, additionalProps = {}) {
        this.objectType = 'operator'; // Discriminator property
        ...
        if (validateTypeConstraints) {
            if (type === 'hermitian' && !this.isHermitian()) {
                ...
                const adjointMatrix = Array(this.dimension).fill(null)
                    ...
                    .map((_, j) => math.conj(this.matrix[j][i])));
    ...
    toString(precision = 3) {
        ...
        const paddedRows = rows.map(row => '│ ' + row.split('\t')
     ...
     */
    static createOptimized(matrix, type) {
    ...
    add(other) {
        ...
        const sumMatrix = Array(this.dimension).fill(null)
            .map((_, i) => Array(this.dimension).fill(null)
            .map((_, j) => math.add(math.complex(this.matrix[i][j].re, this.matrix[i][j].im), math.complex(otherMatrix[i][j].re, otherMatrix[i][j].im))));
    ...
    eigenDecompose() {
        ...
        const vectorOperators = vectors.map(v => {
            ...
            const matrix = Array(this.dimension).fill(null)
                .map((_, i) => Array(this.dimension).fill(null)
    ...
    projectOntoEigenspace(eigenvalue, tolerance = 1e-10) {
        ...
        const matchingVectors = vectors.filter((_, i) => Math.abs(values[i].re - eigenvalue.re) < tolerance &&
