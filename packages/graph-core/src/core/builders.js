...
import { empty as emptyGenerator, path as pathGenerator } from 'graphology-generators/classic';
import { erdosRenyi } from 'graphology-generators/random';
import { GraphologyAdapter } from './GraphologyAdapter';
export function empty(nodeCount) {
...
export function complete(nodeCount) {
    const adapter = new GraphologyAdapter();
...
export function path(nodeCount) {
    const adapter = new GraphologyAdapter();
...
export function random(nodeCount, probability) {
    const adapter = new GraphologyAdapter();
...
export function randomSparse(nodeCount, probability) {
    const adapter = new GraphologyAdapter();
...
// === Lattice Generators ===
export function lattice1D(length) {
    const adapter = new GraphologyAdapter();
    const graph = new Graphology();
    for (let i = 0; i < length; i++) {
        const latticePosition = { i, j: 0 };
...
export function lattice2D(width, height) {
    const adapter = new GraphologyAdapter();
    const graph = new Graphology();
...
export function lattice1DPeriodic(length) {
    const adapter = new GraphologyAdapter();
    const graph = new Graphology();
    // Create nodes with logical lattice identifiers (NO coordinates)
    for (let i = 0; i < length; i++) {
...
export function lattice2DPeriodic(width, height) {
    const adapter = new GraphologyAdapter();
    const graph = new Graphology();
    // Create nodes with logical lattice identifiers (NO coordinates)
    for (let i = 0; i < width; i++) {
...
export function triangularLattice(width, height) {
    const adapter = new GraphologyAdapter();
    const graph = new Graphology();
    // Create nodes with logical lattice identifiers (NO coordinates)
    for (let i = 0; i < width; i++) {
